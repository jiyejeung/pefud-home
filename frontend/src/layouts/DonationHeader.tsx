...
const DonationHeader = ({ backgroundColor = '' }: PropsType) => {
  ...
  /*
      ...
      d : { min: '1280px' },
  */
  // ${scrolled ? 'bg-custom-white' : 'bg-transparent'}
  return (
    <>
      <header className={`w-100% flexRow px-[4rem] l:px-[3rem] p:px-[2rem] t:px-[2vw] backdrop-blur-sm l:bg-coral p:bg-yellow t:bg-green fixed transition-all-300 z-200 ${scrolled ? 'py-[0.3rem] l:py-[0.5rem] p:py-[0.3rem] t:py-[0.5vw] d:bg-custom-white' : 'py-[1.5rem] l:py-[1rem] p:py-[0.7rem] t:py-[1vw] d:bg-transparent '}`} style={{ backgroundColor }}>
        <nav className={`max-w-[120rem] w-100% rounded-[0.5rem] py-[1.5rem] px-[2rem] flexRow justify-between transition-all-300 bg-transparent`}>
          ...
          <div className='flexRow'>
            <button className='h-[4.5rem] text-[2rem] leading-[2rem] pt-[0rem] font-impact font-[800] border rounded-[0.5rem] px-[2rem] cursor-pointer' onClick={handleClickConnectWallet}>
              Connect Wallet
            </button>
            <button className='h-[4.5rem] border rounded-[0.5rem] px-[2rem] flexRow overflow-hidden cursor-pointer' onClick={handleClickConnectEonNetwork}>
            ...
            <div className='h-[4.5rem] border rounded-[0.5rem] flexRow overflow-hidden pr-[2rem] pl-[1rem]'>
            ...
            {/* <div className='w-[5rem] h-[4.5rem] pl-[0.1rem] flexRow rounded-[0.5rem] relative border cursor-pointer'>
              <div className='w-[2.5rem] h-[2.5rem] bg-center bg-contain bg-no-repeat' style={{ backgroundImage: 'url("/images/hamburger.png")' }}></div>
              <div className='pt-[0.2rem] absolute bottom-[-4.9rem] right-[0rem] h-[4.5rem] w-[27.7rem]  border text-[2rem] leading-[2rem] font-[800] font-impact rounded-[0.5rem] flexRow pb-[0rem] bg-white'>Disconnect Wallet</div>
            </div> */}
          </div>
        </nav>
      </header>
